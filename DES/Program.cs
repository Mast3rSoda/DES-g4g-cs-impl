...
string encrypt(string pt, List<string> rkb, List<string> rk)
{
    ...
    Console.WriteLine("After splitting: L0=" + bin2hex(left) + " R0=" + bin2hex(right));
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        for (int y = 0; y < 8; y++)
        {
            int row = 2 * (x[y * 6] - '0') + (x[y * 6 + 5] - '0');
            int col = 8 * (x[y * 6 + 1] - '0') + 4 * (x[y * 6 + 2] - '0') + 2 * (x[y * 6 + 3] - '0') + (x[y * 6 + 4] - '0');
            int val = s[y, row * 16 + col];
            op += val / 8;
            val = val % 8;
            op += val / 4;
            val = val % 4;
        ...
        Console.WriteLine("Round " + (i + 1) + " " + bin2hex(left) + " "
             + bin2hex(right) + " " + rk[i]);
    }
